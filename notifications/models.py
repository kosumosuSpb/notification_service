...
class Client(models.Model):
    ...
    utc_regex = RegexValidator(regex=r'^[+-]?([01]\d?)|(\d)',  # примитивный валидатор часового пояса клиента
                               message='Часовой пояс должен быть от "-12" до "+12" или "0"')
    ...
    def save(self, *args, **kwargs):
        ...
        # определяем оператора (если такого ещё нет - создаём)
        self.operator, created = Operator.objects.get_or_create(code=code, defaults={'name': code})
...
class Message(models.Model):
    ...
    sent = models.BooleanField(default=False)  # отправлено? поле статуса. Можно сделать более развёрнутым
